...
def get_facerender_data(coeff_path, pic_path, first_coeff_path, audio_path, 
    ...
    reordered_target_semantics = np.empty_like(target_semantics_np)

    for i in range(0, frame_num, batch_size):
        for j in range(batch_size):
            if i + j < frame_num:
                reordered_target_semantics[i // batch_size + j * frame_num // batch_size] = target_semantics_np[i + j]

    target_semantics_np = reordered_target_semantics.reshape(batch_size, -1, target_semantics_np.shape[-2],
