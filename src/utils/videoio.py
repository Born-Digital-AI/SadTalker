...
def fill_images(imgs, base_img_path):
    ...
    base_width = base_width * overlay_height // base_height if is_wide_format else overlay_width
    base_height = overlay_height if is_wide_format else base_height * overlay_width // base_width

    base_img = base_img.resize((base_width, base_height))

    overlay_images = []

    for frame in tqdm(imgs, "Overlay images gen:"):
            ...
            for y in range(overlay_height):
                for x in range(overlay_width):
                    ...
                    if dist_y_top > border_top and dist_y_bottom > border_bottom:
                        ...
                        mask[y][x] = 255 * (dist_y_top / border_top)
                    else:
                        mask[y][x] = 255 * (dist_y_bottom / border_bottom)
        # wide video
        else:
            border = 200
            for y in range(overlay_height):
