...
def main(args):
    ...
    save_dir = os.path.join(
        args.result_dir,
        (
            final_vid_name.split(".")[0]
            if final_vid_name
            else strftime("%Y_%m_%d_%H.%M.%S")
        ),
    )
    ...
    batch = get_data(
        ...
        still=args.still,
        idlemode=args.idlemode,
        length_of_audio=args.len,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bg_image",
        default=None,
        help="path to background image (wide or portrait mode)",
    )
    parser.add_argument(
        "--ref_eyeblink",
        default=None,
    ...
    parser.add_argument(
        "--final_vid_name", default=None, help="name of the final generated video"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--expression_scale", type=float, default=1.0, help="expression scale"
    )
    parser.add_argument(
        "--head_motion_scale", type=float, default=1.0, help="head motion scale"
    )
    parser.add_argument(
        "--input_yaw",
        nargs="+",
    ...
    parser.add_argument(
        "--idlemode", action="store_true", help="makes video non-speaking"
    )
    parser.add_argument(
        "--len", type=int, default=0, help="length of silent audio when using idlemode"
    )
    parser.add_argument(
        "--preprocess",
        default="crop",
